...
\begin{enumerate}
    ...
    \item $\mathbb{R}^n$の集合列$\{K_i\}_{i=1}^\infty$が$K_i\supset K_{i+1}\ (\forall i\in\mathbb{N})$を満たすとき，以下の問に答えよ。
        \begin{enumerate}\renewcommand{\labelenumii}{(\arabic{enumii})}
            ...
            \item すべての$i\in\mathbb{N}$に対し$K_i$がcompact集合でないとき$\displaystyle\bigcap_{i=1}^\infty K_i=0$となる例を1つ示せ。
        \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item 以下の問にそれぞれ答えよ。
        \begin{enumerate}\renewcommand{\labelenumii}{(\arabic{enumii})}
            ...
            \item $f(x,y,z)=(e^{y+z}-1)\cos x+z^2$，$g(x,y,z)=\sin x\cos y+\cos x\sin z$とおく。原点$(0,0,0)$の近傍で$f=g=0$に対する院関数$y=y(x)$，$z=z(x)$が存在することを示せ。このとき，$y'(0)$，$z'(0)$を求めよ。
        \end{enumerate}
...
\begin{enumerate}
    ...
    \item
        \begin{enumerate}\renewcommand{\labelenumii}{(\arabic{enumii})}
            ...
            \[
            \exists\lambda:\mathbb{R}^n\to\mathbb{R}^m:\textrm{linear}\ \textrm{s.t.}\ \forall\epsilon>0,\exists\delta_0>0\ \textrm{s.t.}\ |h|_n<\delta_0 \Rightarrow \frac{|f(a+h)-f(a)-\lambda(h)|_m}{|h|_n}<\epsilon
            \]
            このことと，線形写像の有界性から
            \[
            ...
            \end{split}
            \]
            よって，$\displaystyle\delta:=\min\{\delta_0,\frac{\epsilon}{\epsilon+M}\}$とすれば，
            \[
    ...
    \item
        \begin{enumerate}\renewcommand{\labelenumii}{(\arabic{enumii})}
            \item 背理法により示す。すなわち，すべての$i\in\mathbb{N}$に対し$K_i$がcompact集合ならば$\displaystyle\bigcap_{i=1}^\infty K_i=0$であると仮定する。このとき，$\displaystyle K_1\subset \bigcup_{i=1}^\infty K_i^c$となるが，$K_1$はcompact故，ある$N\in\mathbb{N}$に対して$\displaystyle K_1\subset \bigcup_{i=1}^N K_i^c$とできる。これと$\displaystyle \bigcup_{i=1}^N K_i^c=K_N^c$であることから$K_N\subset K_1^c$となる。これと$K_N\subset K_1$をあわせると$K_N\subset K_1\cup K_1^c=\emptyset$となり$\displaystyle\bigcap_{i=1}^\infty K_i=0$に矛盾。
