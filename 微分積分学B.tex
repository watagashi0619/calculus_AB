...
\begin{note}
	この講義ノートは，授業の板書をもとに編集者が勝手にレイアウトを変更している箇所があります。より実際の授業の板書に近いノートを他の方が別のファイル（2018年度版）で上げていますので，そちらも合わせて見ていただいた方が良いかと思われます。（編集者は授業に出ないことがよくあるので，よく参考にしています。なお，そちら側で誤っていると思われる箇所はこちら側で極力直したつもりです。）なお，授業内容，板書は2018年度版と変化はありません。
\end{note}
...
\begin{proof}
	$A$の境界が関数$\chi_A$の不連続点の全体と一致することを示せば定理\ref{th3.8}よりOK。
	\begin{itemize}
...
\blfootnote{
\begin{note}
定理\ref{th3.8}によれば，測度0ならばリーマン可積分である。ディリクレ関数はリーマン可積分でない（ディリクレ関数は有理数で1，無理数で0をとる関数）。不連続点はすべての点である。定理\ref{th3.9}によれば積分できないことがわかる。
\end{note}
}
...
\[
\begin{split}
	V&=2\int_{\theta=0}^{\theta=2\pi}\left(\int_{r=0}^{r=a}\sqrt{a^2-r^2}rdr\right)d\theta\\
...
\footnotetext{$x_1,x_2,x_3$を$z,x,y$とすれば物理学でみられる極座標変換に対応する。}
...
\begin{split}
	...
	&=\lim_{n\to\infty}\int_{\theta=0}^{\theta=\pi}\int_{r=\frac{1}{n}}^{r=1}2r\log{r} drd\theta\\
...
\begin{ans}
	...
	\]
    \footnotetext{
    正方形を$y=x$で分割した2つの直角三角形とみて，$y=x$の上側部分の2倍
    }
\end{ans}
...
\begin{example}\
	...
	\item $\displaystyle\frac{\partial}{\partial x}\left(\frac{2x}{(x+y)^2}\right)=-\frac{\partial}{\partial y}\left(\frac{2y}{(x+y)^2}\right)=\frac{2y-2x}{(x+y)^3}=f(x,y)$
		を用いて
		\[
...
\begin{ans}\
    ...
    \]
    \begin{note}
        これは$\displaystyle f(x,y)=\frac{2(x-y)}{(x+y)^3}$が（$[0,1]\times[0,1]$上）可積分でないことを示している。
    \end{note}
    \item
	\[
	\begin{split}
		...
		&=\int_b^1\left(-\frac{2}{(1+y)^2}+\frac{2a}{(a+y)^2}\right)dy\\
...
\footnotetext{
$\displaystyle \Gamma(p)=\int_0^\infty e^{-x}x^{p-1} dx = 2\int_0^\infty e^{-x^2}x^{2p-1} dx$\\
$\displaystyle B(p,q)=\int_0^1 x^{p-1}(1-x)^{q-1}dx=2\int_0^{\frac{\pi}{2}}\sin^{2p-1}\theta\cos^{2q-1}\theta d\theta$
}
...
\begin{proof}
	...
	&=4\left(\int_0^\infty x^{2p-1}e^{-x^2}dx\right)\left(\int_0^\infty y^{2q-1}e^{-y^2}dy\right)\\
...
\footnotetext{
2次形式についての補足：例えば\\
$x^2+2y^2+3z^2-2xy-4yz+6zx=(x,y,z)A\begin{pmatrix} x \\ y \\ z\end{pmatrix}$に対して$A$は
\[
A=
\begin{pmatrix}
	1 & -1 & 3 \\
...
\end{pmatrix}
\]
となる。第$i$列，第$j$列が$1,2,3$に対して$x,y,z$に対応している。$i=j$のところは係数そのまま，$i\neq j$のところは係数の半分の値が入る。\\
または，シルベスターの方法（慣性法則）を用いる。
}
...
\begin{ans}\
    \begin{enumerate}\renewcommand{\labelenumi}{(\arabic{enumi})}
        ...
        \footnotetext{$f:\mathbb{R}^2\to\mathbb{R}^2:f(\begin{pmatrix} u \\ v \end{pmatrix})=P\begin{pmatrix} u \\ v \end{pmatrix}$，線形写像の微分は表現行列である。}
        この変換のヤコビ行列式は$P$である。直交行列の行列式の値は$\pm 1$であるので，それの絶対値は$1$\footnotemark。
        \footnotetext{つまり$dydx=|P|dudv=dudv$となることに注意。}
        フビニの定理を用いて
        \[
        \begin{split}
        ...
        \]
        \footnotetext{計算には
        $\displaystyle \int_{-\infty}^\infty e^{-\alpha x^2}dx=\frac{1}{\sqrt{\alpha}} \int_{-\infty}^\infty e^{-u^2}du=\sqrt{\frac{\pi}{\alpha}}$を用いればよい。}
    \end{enumerate}
\end{ans}
